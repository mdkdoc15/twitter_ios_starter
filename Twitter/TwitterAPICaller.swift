...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favTweet(tweeetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url =  "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweeetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unfavTweet(tweeetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func rtTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func unRtTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
